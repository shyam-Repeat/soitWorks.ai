...
- **Supabase Integration:** Secure user authentication and persistent history of all analyses.
...
- `backend/server.ts`: Express + Vite middleware gateway that handles scraping, AI orchestration, Supabase APIs, and shared instructions.
- `backend/lib/*.ts`: Supabase helpers (`db.ts`, `supabase.ts`) plus `backend/utils/buyerIntentScanner.ts`.
- `backend/scraper.ts` + `backend/instagram_scrapling.py`: Scrapling + Apify hybrid scraping stack.
- `supabase/` (optional): Recommended place for SQL migrations / policies if you manage Supabase locally.
- `src/App.tsx`, `src/components`, `src/hooks`: Vite/React premium dashboard UI accessed via the `npm run dev` workflow.
- `.devcontainer/`: Codespaces setup that wires up Supabase credentials, the scraper, and Vite for instant onboarding.

---

## 🛠️ Getting Started (Codespaces / Local)

1. **Backend environment:** Copy `backend/.env.example` to `backend/.env` and fill in the keys (`GEMINI_API_KEY`, Supabase `SUPABASE_URL` + keys, `APIFY_API_TOKEN`, scraper toggles, etc.). This keeps the server-side secrets next to the backend code and is automatically loaded before the Express routes spin up.
...
4. **Start the stack:** Run `npm run dev` (starts the backend server with Vite middleware; the UI is available on port 5173 in Codespaces).
5. **Access:** Visit the forwarded dev port (e.g., `https://<codespace>.preview/...`) or, when deploying the frontend to Vercel, the published Vercel URL will consume the backend through `VITE_API_BASE_URL`.
